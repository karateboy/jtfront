<template>
  <div class="p-col-12" style="padding: 0">
    ...
    >
      ...
      <Column field="job_order.customer_po_info" header="customer_po" sortable="true" filterMatchMode="contains">
        <template #body="job">
            {{job.data.job_order.customer_po_info}}
        </template>
        <template #filter>
          <InputText type="text" v-model="filters['job_order.customer_po_info']" class="p-column-filter"/>
        </template>
      </Column>      
      ...
      <Column field="order_qty" header="order_qty" sortable="true">
        <template #body="job">
            {{job.data.quantity.order.qty}}
            {{job.data.quantity.order.unit}}
        </template>
      </Column>  
 
       <Column field="work_qty" header="work_qty" sortable="true">
        <template #body="job">
            {{job.data.quantity.job.qty}}
            {{job.data.quantity.job.unit}}
        </template>
      </Column>  
      ...
      <Column field="job_detail.job_due_date" header="order_due" sortable="true" filterMatchMode="contains">
        <template #body="job">
            {{job.data.job_detail.job_due_date | moment("L")}}
        </template>
        <template #filter>
          <InputText type="date" v-model="filters['job_detail.job_due_date']" class="p-column-filter"/>
        </template>
      </Column>
