<template>
  <div class="p-fluid p-grid p-formgrid">
    <Inplace @open="loadData" :closable="true">
      <template #display
        >New Order Entry {{ appDocument.new_jobs.length }}</template
      >
      <template #content>
        <DataTable
          :value="appDocument.new_jobs"
          data-key="work_id"
...
export default {
  ...
  methods: {
    ...mapActions(namespaced, ["POST_JOBS"]),
    saveThis() {
      console.log("saving this from orderentrydetail.vue");
      console.log(this.appDocument.new_jobs);
      this.$store.dispatch(namespaced + "/POST_JOBS", this.appDocument.new_jobs);
      return;
    },
  },
  computed: {
